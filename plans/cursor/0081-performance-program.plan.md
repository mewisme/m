---
name: "0081 Cross-Cutting — Performance and Resource Program"
overview: "Measure and control startup, resolution, network, extraction, linking, script, transform, memory, disk, and process overhead throughout development."
todos:
  - id: paths
    content: "Enumerate hot paths"
    status: pending
  - id: bench
    content: "S/M/L install benches"
    status: pending
  - id: budget
    content: "Threshold file"
    status: pending
  - id: ci
    content: "Regression gate"
    status: pending
  - id: ceiling
    content: "Resource limits"
    status: pending
  - id: docs
    content: "Perf program guide"
    status: pending
isProject: false
---

# 0081 - Cross-Cutting — Performance and Resource Program

## Source contract

Canonical scope lives in [`plans/0081-performance-program.md`](../0081-performance-program.md). Do not expand scope in this Cursor plan; update the repo plan and regenerate.

## Done when

- All required tests pass on supported operating systems.
- No unresolved correctness, integrity, or data-loss issue remains.
- Public behavior and intentional deviations are documented.
- The next dependent MVP can consume stable interfaces without reaching into internals.

## Out of scope

- Do not implement features assigned to later indexed MVPs.
- Do not silently diverge from a documented compatibility contract.
- Do not optimize by weakening integrity, determinism, or crash safety.

## Predecessors

0008, 0010

Block implementation until predecessors are merged and meet their own exit criteria.

## Implementation steps

1. Confirm predecessors satisfied: 0008, 0010
2. Read source contract `plans/0081-performance-program.md` and `0003`/`0007`/`0008` as required.
3. Implement packages: benchmarks/, internal/perf, docs/performance
4. Add fixtures and tests listed in the source contract.
5. Run focused `go test` for touched packages, then applicable integration fixtures.
6. Update feature inventory / docs when public behavior changes.
7. Provide AI-agent handoff evidence (6 items in source contract).

## Verification

1. Baselines recorded for critical paths
2. CI fails on budget breach without waiver
3. Cold/warm separated in reports

Suggested commands (once code exists):

```powershell
go test ./benchmarks//... -count=1
```

Adjust package paths to those actually created. Always include a clean-home fixture test for install-family work.

## Handoff

Before submitting work provide:

1. Behavior summary and compatibility target
2. Files and public interfaces changed
3. Test/benchmark/static-analysis commands
4. Known gaps and platform limits
5. Determinism evidence for generated files
6. Rollback note for persistent-format changes
